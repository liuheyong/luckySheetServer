...
public class RecordDataUpdataHandle extends BaseHandle implements IRecordDataUpdataHandle {
    ...
    public boolean batchUpdateCellDataValue(List<String> block_ids, List<GridRecordDataModel> models) {
        try {
            ...
            String sql = "insert into " + JfGridConfigModel.TABLENAME + " (id,block_id,`index`,list_id,status,`order`,json_data,is_delete) values " + "( ?,?,?,?,?,?,?,0)";
    ...
    public boolean updateJsonbForInsertNull(JSONObject query, String word, JSONObject db, Integer position, String words) {
        try {
            ...
            String createSql = "update " + JfGridConfigModel.TABLENAME + " set json_data=json_set(json_data,\"$." + key + "\",CAST('" + newObj.getString(key) + "' AS JSON)) where 1=1 " + condition + " and JSON_CONTAINS_PATH(jsontest,'one',\"$." + key + "\")=0";
    ...
    public boolean updateJsonbForSetNull(JSONObject query, String word, JSONObject db, Integer position) {
        try {
            ...
            String createSql = "update " + JfGridConfigModel.TABLENAME + " set json_data=json_set(json_data,\"$." + word + "\",CAST('[]' AS JSON)) where 1=1 " + condition + " and JSON_CONTAINS_PATH(jsontest,'one',\"$." + word + "\")=0";
            log.info("createSql:{}", createSql);
            luckySheetJdbcTemplate.update(createSql, arr.toArray());
