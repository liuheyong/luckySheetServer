...
public class RecordDataInsertHandle extends BaseHandle implements IRecordDataInsertHandle {
    ...
    public String insert(GridRecordDataModel pgModel) {
        ...
        String sql = "insert into " + JfGridConfigModel.TABLENAME + " (id,block_id,index,list_id,status,json_data,\"order\",is_delete) values " + " (nextval('luckysheet_id_seq'),?,?,?,?,?,?,0)";
        try {
    ...
    public String InsertIntoBatch(List<GridRecordDataModel> models) {
        String sql = "insert into " + JfGridConfigModel.TABLENAME + " (id,block_id,row_col,index,list_id,status,\"order\",json_data,is_delete) values " + " (nextval('luckysheet_id_seq'),?,?,?,?,?,?,?,0)";
